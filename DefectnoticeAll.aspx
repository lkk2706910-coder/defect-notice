...
<head>
    ...
    <style>
        ...
        /* dn-table numeric alignment */
        .dn-table td.num, .dn-table th.num { text-align:right; }
        ...
        .tab-btn:hover { background: rgba(99, 179, 237, 0.10); }
...
<body>
        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="tab-bar">
                    ...
                    <button type="button" class="tab-btn" data-tab="NewFab24">CMP/Cu 類 24hr</button>
                    <button type="button" class="tab-btn" data-tab="Pivot">Pivot 分析</button>
                </div>
            </div>
            <div class="card-body">
                <div id="paneDash" class="tab-pane">
                    <!-- ===== Dashboard tab (was Defectnotice.aspx) ===== -->
                    <div class="subtitle">依 EQPID 篩選 + DataDate 範圍；結果依 EQPTYPE 分群顯示（重複資料會全部列出）</div>

                    <!-- 置頂公告（後端會填入內容） -->
                    <asp:PlaceHolder ID="phDashNotice" runat="server"></asp:PlaceHolder>
                    ...
                    <div class="layout1 sections" id="layout1">
                        <asp:PlaceHolder ID="phDashTable" runat="server"></asp:PlaceHolder>
                    </div>
                ...
                <div id="paneNotice24" class="tab-pane" style="display:none;">
                    <!-- ===== 24hr Notice tab (was Defectnotice2.aspx) ===== -->
                    <div class="subtitle">依 EQPID 篩選 + DataDate 範圍；結果依 EQPTYPE 分群顯示（重複資料會全部列出）</div>

                    <!-- 置頂公告（後端會填入內容） -->
                    <asp:PlaceHolder ID="phNotice24Notice" runat="server"></asp:PlaceHolder>
                    ...
                    <div class="layout1 sections" id="layout1">
                        <asp:PlaceHolder ID="phNotice24Table" runat="server"></asp:PlaceHolder>
                    </div>
                ...
                <div id="paneNewFab24" class="tab-pane" style="display:none;">
                    <!-- ===== CMP/Cu 24hr tab (was Defectnotice3.aspx) ===== -->
                    <div class="subtitle">依 EQPID 篩選 + DataDate 範圍；結果依 EQPTYPE 分群顯示（重複資料會全部列出）</div>

                    <!-- 置頂公告（後端會填入內容） -->
                    <asp:PlaceHolder ID="phNewFab24Notice" runat="server"></asp:PlaceHolder>
                    ...
                    <div class="toolbar toolbar-compact">
                        <div class="pill-row">
                            ...
                            <div class="pill">
                                <span class="chip">Tool:</span>
                                <button type="button" class="btn jump" data-toolprefix="ALDOX-" style="margin:6px 6px;">ALDOX</button>
                                <button type="button" class="btn jump" data-toolprefix="CULKCVD-" style="margin:6px 6px;">CULKCVD</button>
                                <button type="button" class="btn jump" data-toolprefix="CUSILPE-" style="margin:6px 6px;">CUSILPE</button>
                                <button type="button" class="btn jump" data-toolprefix="DARC-" style="margin:6px 6px;">DARC</button>
                                <button type="button" class="btn jump" data-toolprefix="HKG-" style="margin:6px 6px;">HKG</button>
                                <button type="button" class="btn jump" data-toolprefix="SILPE-" style="margin:6px 6px;">SILPE</button>
                            </div>
                    ...
                    <div class="layout1 sections" id="layout1">
                        <asp:PlaceHolder ID="phNewFab24Table" runat="server"></asp:PlaceHolder>
                    </div>
                ...
                <div id="panePivot" class="tab-pane" style="display:none;">
                    <!-- ===== Pivot tab (was Defectnotice4.aspx) ===== -->
                    <div class="subtitle">依 EQPID 篩選 + DataDate 範圍；結果依 EQPTYPE 分群顯示（重複資料會全部列出）</div>
                    ...
                    <div class="toolbar toolbar-compact">
                        <div class="pill-row">
                            ...
                            <div class="pill">
                                ...
                                <label class="chip"><input type="radio" name="pivotmode" class="pivotmode" value="GROUP" checked /> by Defect pattern</label>
                                <label class="chip"><input type="radio" name="pivotmode" class="pivotmode" value="ALL" /> not by Defect pattern</label>
                            </div>
                    ...
                    <div class="layout1 sections" id="layout1">
                        <asp:PlaceHolder ID="phPivotTable" runat="server"></asp:PlaceHolder>
                    </div>
    ...
    <script>
        ...
        (function(){
            ...
            const panes = document.querySelectorAll('.tab-pane[id]');
        ...
        /* === Dashboard IIFE === */
        (function(){
            const pane = document.getElementById('paneDash');
            if (!pane) return;
            ...
            const chart = new Chart(ctx, {
                ...
                data: {
                    labels: trend.labels,
                    datasets: ['SACVD', 'NISACVD'].map(makeDataset)
                },
                options: {
                    responsive: true,
            ...
            function renderEqWeekChart() {
                const selected = new Set([...pane.querySelectorAll('input.eqpid:checked')].map(x => x.value));
            ...
            function getRangeValue() {
                const r = pane.querySelector('input.range:checked');
                return r ? r.value : '90';
            }
            ...
            function getFabValue() {
                const r = pane.querySelector('input.fab:checked');
            ...
            function syncTable(selected, monthSet) {
                ...
                pane.querySelectorAll('#countsTable .sec-row').forEach(tr => {
                    ...
                    if (isChild) {
                        ...
                        const parentRow = pane.querySelector(`#countsTable tr.parent-row[data-section="${parent}"]`);
                ...
                pane.querySelectorAll('#countsTable .month-col').forEach(td => {
                    const m = td.getAttribute('data-month');
                    td.style.display = (!monthSet || monthSet.has(m)) ? '' : 'none';
                });
            }

            let activeSubEqpid = null; // e.g. SACVD-04

            function setChartType(t) {
                chartType = t;

                const trendSection = pane.querySelector('#trendChart')?.closest('.section');
                const pieSection = pane.querySelector('#trendPieSection');
            ...
            function renderToolPieCharts() {
                ...
                for (const sec of order) {
                    ...
                    // title above pie
                    const titleEl = pane.querySelector('#pie_title_' + sec);
            ...
            function applyCardFilter(selected) {
                const fab = getFabValue();
                const qLot = (pane.querySelector('#qLot')?.value || '').trim().toLowerCase();
                const qEqp = (pane.querySelector('#qEqp')?.value || '').trim().toLowerCase();
                const startDate = getRangeStartDate();

                // 先依 top checkbox 決定 section 顯示
                pane.querySelectorAll('.sections > [data-section]').forEach(sectionEl => {
                ...
                pane.querySelectorAll('.item[data-sub-eqpid]').forEach(item => {
            ...
            pane.addEventListener('click', (ev) => {
                ...
                if (childRow) {
                    ...
                    if (sec) {
                        pane.querySelectorAll('input.eqpid').forEach(cb => { cb.checked = (cb.value === sec); });
                    }
                ...
                if (parentRow) {
                    ...
                    pane.querySelectorAll(`#weeklySection tr.week-child[data-parent-section="${sec}"]`).forEach(tr => {
                        tr.style.display = expanded ? 'none' : '';
                    });
                }
            ...
            function syncCountsTableVisibility() {
                const grain = pane.querySelector('input.countgrain:checked')?.value || 'month';
                const monthEl = pane.querySelector('#monthlySection');
                const weekEl = pane.querySelector('#weeklySection');
            ...
            function syncCountsTableColumns() {
                ...
                if (grain === 'month') {
                    const monthSet = getRangeMonthsSet();
                    pane.querySelectorAll('#monthlySection .month-col').forEach(td => {
                        const m = td.getAttribute('data-month');
                        td.style.display = (!monthSet || monthSet.has(m)) ? '' : 'none';
                    ...
                    pane.querySelectorAll('#weeklySection tr.week-child[data-is-p56]').forEach(tr => {
                        const isP56Child = tr.getAttribute('data-is-p56') === '1';
                        const parent = tr.getAttribute('data-parent-section');
                        const parentRow = parent ? pane.querySelector(`#weeklySection tr.week-parent[data-section="${parent}"]`) : null;
                        const expanded = parentRow && parentRow.getAttribute('data-expanded') === '1';
                    ...
                    pane.querySelectorAll('#weeklySection .week-col').forEach(td => {
            ...
            function renderPerformanceTable() {
                const tbody = pane.querySelector('#perfTable tbody');
                if (!tbody) return;

                const { secMap, secTotal } = calcPerfBySection();
                const secOrder = ['SACVD', 'NISACVD'];
                ...
                const hint = pane.querySelector('#perfHint');
                if (hint) {
            ...
            pane.addEventListener('click', (ev) => {
                ...
                const input = pane.querySelector('#qEqp');
            ...
            function syncVisibility() {
                ...
                const ct = pane.querySelector('input.charttype:checked')?.value || 'bar';
                if (ct === 'pie') {
                    renderToolPieCharts();
                }
            ...
            function wireCardToggle() {
                pane.querySelectorAll('.item .item-collapsed').forEach(el => {
            ...
            pane.querySelectorAll('input.fab').forEach(rb => rb.addEventListener('change', syncVisibility));
            pane.querySelectorAll('input.eqpid').forEach(cb => cb.addEventListener('change', syncVisibility));
            pane.querySelectorAll('input.range').forEach(rb => rb.addEventListener('change', syncVisibility));
            ...
            // Chart type (Bar/Pie)
            pane.querySelectorAll('input.charttype').forEach(rb => rb.addEventListener('change', () => {
                const v = pane.querySelector('input.charttype:checked')?.value || 'bar';
                setChartType(v);
            }));

            // Search：打字即時觸發過濾
            const qLotEl = pane.querySelector('#qLot');
            const qEqpEl = pane.querySelector('#qEqp');
            ...
            pane.querySelector('#btnReset')?.addEventListener('click', () => {
                // FAB default = ALL
                pane.querySelectorAll('input.fab').forEach(rb => { rb.checked = (rb.value === 'ALL'); });
                // EQPID default = all checked
                pane.querySelectorAll('input.eqpid').forEach(cb => { cb.checked = true; });
                // Range default = 30D
                pane.querySelectorAll('input.range').forEach(rb => { rb.checked = (rb.value === '30'); });

                // Counts grain default = month
                pane.querySelectorAll('input.countgrain').forEach(rb => { rb.checked = (rb.value === 'month'); });

                // chart type UI default
                pane.querySelectorAll('input.charttype').forEach(rb => { rb.checked = (rb.value === 'bar'); });
                setChartType('bar');
                destroyPieCharts();
                ...
                activeSubEqpid = null;
                pane.querySelectorAll('#countsTable tr.parent-row').forEach(tr => tr.setAttribute('data-expanded', '0'));
                pane.querySelectorAll('#countsTable tr.child-row').forEach(tr => tr.style.display = 'none');
                pane.querySelectorAll('#weeklySection tr.week-parent').forEach(tr => tr.setAttribute('data-expanded', '0'));
                pane.querySelectorAll('#weeklySection tr.week-child').forEach(tr => tr.style.display = 'none');

                // collapse expanded cards
                pane.querySelectorAll('.item.expanded').forEach(el => el.classList.remove('expanded'));

                syncVisibility();
            });

            // open performance popup
            pane.querySelector('#btnPerf')?.addEventListener('click', () => {
                // 確保最新計算結果
                renderPerformanceTable();
...
<div class="container" style="max-width:1280px; margin:16px auto; padding:0 16px;">
  <div class="card">
    ...
    <div class="card-body">
      <div style="overflow:auto;">${pane.querySelector('#perfTable')?.outerHTML || ''}</div>
      <div class="muted" style="margin-top:10px; font-size:12px;">(This window is read-only)</div>
            ...
            function applyLayoutMode(mode) {
                const l1 = pane.querySelector('#layout1');
                const l2 = pane.querySelector('#layout2');
                const btn = pane.querySelector('#btnLayout');
                if (!l1 || !l2 || !btn) return;
                ...
                if (mode === 2) {
                    ...
                    const countsMonth = pane.querySelector('#monthlySection');
                    const countsWeek = pane.querySelector('#weeklySection');
                    const cWrap = pane.querySelector('#layout2Counts');
                    if (cWrap) {
                        cWrap.innerHTML = '';
                    ...
                    const secWrap = pane.querySelector('#layout2Sections');
                    if (secWrap) {
                        secWrap.innerHTML = '';
                        pane.querySelectorAll('.section[data-section]').forEach(sec => { secWrap.appendChild(sec); });
                    }
                } else {
                    ...
                    const l1El = pane.querySelector('#layout1');
                    if (l1El) {
                        ...
                        pane.querySelectorAll('#layout2Sections .section[data-section]').forEach(sec => { l1El.appendChild(sec); });
                    }
                }
            ...
            let __layoutMode = 1;
            pane.querySelector('#btnLayout')?.addEventListener('click', () => {
        ...
        /* === Notice24 IIFE === */
        (function(){
            const pane = document.getElementById('paneNotice24');
            if (!pane) return;
            ...
            let activeSubEqpid = null; // e.g. ULKCVD-04
                                                    ...
                                                    function applyCardFilter() {
                                                        ...
                                                        pane.querySelectorAll('.sections > [data-section]').forEach(sectionEl => { sectionEl.style.display = ''; });

                                                        // Table mode: rows are <tr class='dn-row item'>
                                                        pane.querySelectorAll('tr.item[data-sub-eqpid]').forEach(item => {
                                                            const isP56 = item.getAttribute('data-is-p56') === '1';
                                                            if (fab === 'P56' && !isP56) { item.style.display = 'none'; return; }
                                                            if (fab === 'P14' && isP56) { item.style.display = 'none'; return; }
                                                        ...
                                                        const qEqpRaw = (pane.querySelector('#qEqp')?.value || '').trim().toLowerCase();
                                                        const toolPrefix = qEqpRaw.endsWith('-') ? qEqpRaw : '';
                                                        const fabMode = getFabValue();

                                                        pane.querySelectorAll('a.notice-link[data-tool]').forEach(a => {
                                                        ...
                                                        pane.querySelectorAll('button.jump[data-toolprefix]').forEach(b => {
            ...
            pane.querySelector('#layout1')?.classList.add('table-mode');
        ...
        /* === NewFab24 IIFE === */
        (function(){
            const pane = document.getElementById('paneNewFab24');
            if (!pane) return;
            ...
            let activeSubEqpid = null; // e.g. ALDOX-04
        ...
        /* === Pivot IIFE === */
        (function(){
            const pane = document.getElementById('panePivot');
            if (!pane) return;
            ...
            function getPivotMode() {
                const r = pane.querySelector('input.pivotmode:checked');
                return r ? r.value : 'GROUP';
            }

            function applyPivotModeVisibility() {
                const mode = getPivotMode();
                // GROUP: show each pattern section; ALL: show the aggregated section
                pane.querySelectorAll('[data-section="PIVOTPAT"]').forEach(el => {
                    el.style.display = (mode === 'GROUP') ? '' : 'none';
                });
                pane.querySelectorAll('[data-section="PIVOTALL"]').forEach(el => {
                    el.style.display = (mode === 'ALL') ? '' : 'none';
                });
            }

            function applyPivotFabFilter() {
                const fab = getFabValue();
                const pick = (el) => {
                    const all = parseInt(el.getAttribute('data-all') || '0', 10) || 0;
                    const p14 = parseInt(el.getAttribute('data-p14') || '0', 10) || 0;
                    const p56 = parseInt(el.getAttribute('data-p56') || '0', 10) || 0;
                    return (fab === 'P14') ? p14 : (fab === 'P56') ? p56 : all;
                };

                // per-table update (avoid cross-table totals)
                pane.querySelectorAll('table.pivotTable').forEach(tbl => {
                    // body cells
                    tbl.querySelectorAll('td.pivot-cell[data-all]').forEach(td => {
                        td.textContent = String(pick(td));
                    });

                    // row totals
                    tbl.querySelectorAll('td.pivot-total[data-all]').forEach(td => {
                        td.textContent = String(pick(td));
                    });

                    // column totals
                    tbl.querySelectorAll('th.pivot-col-total[data-all]').forEach(th => {
                        th.textContent = String(pick(th));
                    });

                    // grand total (this table only)
                    const gt = tbl.querySelector('th.pivot-grand-total[data-all]');
                    if (gt) gt.textContent = String(pick(gt));

                    // hide/show EQP rows by FAB (this table only)
                    tbl.querySelectorAll('tr[data-pivot-row]').forEach(tr => {
                        const a = tr.querySelector('a.pivot-eqp[data-eqp]');
                        const eqp = (a?.getAttribute('data-eqp') || '').toUpperCase();
                        const isB = eqp.indexOf('-B') >= 0;

                        if (fab === 'P56') {
                            tr.style.display = isB ? '' : 'none';
                        } else if (fab === 'P14') {
                            tr.style.display = isB ? 'none' : '';
                        } else {
                            tr.style.display = '';
            ...
            function setActiveSubEqpid(sub) {
                ...
                pane.querySelectorAll('#countsTable tr.child-row').forEach(tr => {
                    tr.style.outline = (tr.getAttribute('data-sub-eqpid') === sub) ? '2px solid rgba(99,179,237,0.55)' : '';
                                ...
                                function syncVisibility() {
                // FAB filter + search inputs
                applyCardFilter();
                applyPivotModeVisibility();
                applyPivotFabFilter();
            }
            ...
            // ===== Table mode filters (like screenshot) =====
                                ...
                                pane.querySelectorAll('input.fab').forEach(rb => rb.addEventListener('change', syncVisibility));
                                pane.querySelectorAll('input.pivotmode').forEach(rb => rb.addEventListener('change', syncVisibility));
            ...
            // Pivot EQP click: filter bottom table by EQPID (toggle)
            pane.addEventListener('click', (ev) => {
                const a = ev.target && ev.target.closest ? ev.target.closest('a.pivot-eqp[data-eqp]') : null;
                if (!a) return;
                const eqp = (a.getAttribute('data-eqp') || '').trim();
                const input = pane.querySelector('#qEqp');
                if (!input) return;

                const cur = (input.value || '').trim();
                if (cur.toLowerCase() === eqp.toLowerCase()) input.value = '';
                else input.value = eqp;
