...
public partial class DefectnoticeAll : System.Web.UI.Page
{
    ...
    private void RenderDashboard()
    {
        ...
        // Per spec: 2 sections (SACVD/NISACVD), card-style vertical list
        string sql = @"
SELECT TOP (1000)
    [DataDate],
    [ISSUEDATE],
...
WHERE
    (
        EQPID LIKE 'SACVD-%' OR EQPID LIKE '%SACVD%'
...
";
        ...
        using (SqlCommand cmd = new SqlCommand(sql, conn))
        {
            ...
            using (SqlDataReader reader = cmd.ExecuteReader())
            {
                ...
                var sections = new[] { "SACVD", "NISACVD" };
                ...
                foreach (var sec0 in new[] { "SACVD", "NISACVD" })
                {
                ...
                foreach (var r in rows)
                {
                    ...
                    // ALL
                    int v;
                    if (!trendAll.ContainsKey(sec)) continue;
                ...
                foreach (var r in rows)
                {
                    ...
                    int v;
                    if (!weeklyCounts.ContainsKey(sec)) continue;
                ...
                foreach (var sec in new[] { "SACVD", "NISACVD" })
                {
                ...
                js.Append("],seriesAll:{");
                AppendTrendSeries(js, labels, trendAll, new[] { "SACVD", "NISACVD" });
                js.Append("},seriesP14:{");
                AppendTrendSeries(js, labels, trendP14, new[] { "SACVD", "NISACVD" });
                js.Append("},seriesP56:{");
                AppendTrendSeries(js, labels, trendP56, new[] { "SACVD", "NISACVD" });
                js.Append("}};</script>");
                ...
                foreach (var r in rows)
                {
                    ...
                    // level1
                    if (!monthCountsLevel1.ContainsKey(sec)) continue;
                    int v1;
                ...
                // Keep only the 2 main groups (SACVD/NISACVD)
                ...
                foreach (var kv in recentEqpidCount)
                {
                    ...
                    string tool = kv.Key ?? "";
                    bool ok = tool.StartsWith("SACVD-", StringComparison.OrdinalIgnoreCase)
                           || tool.StartsWith("NISACVD-", StringComparison.OrdinalIgnoreCase);
                    if (!ok) continue;
                ...
                foreach (var r in rows)
                {
                    ...
                    for (int i = 0; i < parts.Length; i++)
                    {
                        ...
                        if (!todayB.ContainsKey(sec)) continue;
                        var cur = todayB[sec];
                ...
                foreach (var section in sections)
                {
                    ...
                    {
                        // search input only on the first section (avoid id collisions)
...
";
        ...
        using (SqlCommand cmd = new SqlCommand(sql, conn))
        {
            ...
            using (SqlDataReader reader = cmd.ExecuteReader())
            {
                ...
                js.Append("],seriesAll:{");
                AppendTrendSeries(js, labels, trendAll, new[] { "ULKCVD", "TEOSPE", "APF", "BLOKCVD", "NISACVD", "SACVD" });
                js.Append("},seriesP14:{");
                AppendTrendSeries(js, labels, trendP14, new[] { "ULKCVD", "TEOSPE", "APF", "BLOKCVD", "NISACVD", "SACVD" });
                js.Append("},seriesP56:{");
                AppendTrendSeries(js, labels, trendP56, new[] { "ULKCVD", "TEOSPE", "APF", "BLOKCVD", "NISACVD", "SACVD" });
                js.Append("}};</script>");
...
WHERE
    (
        EQPID LIKE 'ALDOX-%' OR EQPID LIKE '%ALDOX%'
        OR EQPID LIKE 'CULKCVD-%' OR EQPID LIKE '%CULKCVD%'
        OR EQPID LIKE 'CUSILPE-%' OR EQPID LIKE '%CUSILPE%'
        OR EQPID LIKE 'DARC-%' OR EQPID LIKE '%DARC%'
        OR EQPID LIKE 'HKG-%' OR EQPID LIKE '%HKG%'
        OR EQPID LIKE 'SILPE-%' OR EQPID LIKE '%SILPE%'
    )
        AND ISSUEDATE >= @StartDate
    AND ISSUEDATE <  @EndDate
ORDER BY [DataDate] DESC;
";
        ...
        using (SqlCommand cmd = new SqlCommand(sql, conn))
        {
            ...
            using (SqlDataReader reader = cmd.ExecuteReader())
            {
                ...
                var sections = new[] { "ALDOX", "CULKCVD", "CUSILPE", "DARC", "HKG", "SILPE" };
                var sb = new StringBuilder();
                ...
                foreach (var sec0 in new[] { "ALDOX", "CULKCVD", "CUSILPE", "DARC", "HKG", "SILPE" })
                {
                    trendAll.Add(sec0, new System.Collections.Generic.Dictionary<string, int>(StringComparer.Ordinal));
                    trendP14.Add(sec0, new System.Collections.Generic.Dictionary<string, int>(StringComparer.Ordinal));
                    trendP56.Add(sec0, new System.Collections.Generic.Dictionary<string, int>(StringComparer.Ordinal));
                ...
                // ===== Weekly table: count by week (Mon-Sun), per section =====
                var weeklyCounts = new System.Collections.Generic.Dictionary<string, System.Collections.Generic.Dictionary<string, int>>(StringComparer.Ordinal);
                                weeklyCounts.Add("ALDOX", new System.Collections.Generic.Dictionary<string, int>(StringComparer.Ordinal));
                weeklyCounts.Add("CULKCVD", new System.Collections.Generic.Dictionary<string, int>(StringComparer.Ordinal));
                weeklyCounts.Add("CUSILPE", new System.Collections.Generic.Dictionary<string, int>(StringComparer.Ordinal));
                weeklyCounts.Add("DARC", new System.Collections.Generic.Dictionary<string, int>(StringComparer.Ordinal));
                weeklyCounts.Add("HKG", new System.Collections.Generic.Dictionary<string, int>(StringComparer.Ordinal));
                weeklyCounts.Add("SILPE", new System.Collections.Generic.Dictionary<string, int>(StringComparer.Ordinal));
                ...
                var weekLabels = new System.Collections.Generic.SortedSet<string>(StringComparer.Ordinal);
                                foreach (var sec in new[] { "ALDOX", "CULKCVD", "CUSILPE", "DARC", "HKG", "SILPE" })
  {
                    foreach (var kv in weeklyCounts[sec]) weekLabels.Add(kv.Key);
                }
                ...
                for (int i = 0; i < rows.Count; i++)
                {
                    ...
                    // short label: ALDOX-B01 => A-B01, CULKCVD-B01 => C-B01, CUSILPE-B01 => U-B01, DARC-B01 => D-B01, HKG-B01 => H-B01, SILPE-B01 => S-B01
                    string shortTool = ToShortToolLabel(sec, tool);
                ...
                js.Append("],seriesAll:{");
                AppendTrendSeries(js, labels, trendAll, new[] { "ALDOX", "CULKCVD", "CUSILPE", "DARC", "HKG", "SILPE" });
                js.Append("},seriesP14:{");
                AppendTrendSeries(js, labels, trendP14, new[] { "ALDOX", "CULKCVD", "CUSILPE", "DARC", "HKG", "SILPE" });
                js.Append("},seriesP56:{");
                AppendTrendSeries(js, labels, trendP56, new[] { "ALDOX", "CULKCVD", "CUSILPE", "DARC", "HKG", "SILPE" });
                js.Append("}};</script>");

                // 先把 JS script 丟到最前面，確保前端圖表可讀到資料
                phNewFab24Table.Controls.Add(new Literal { Text = js.ToString() });
                                ...
                                foreach (var r in rows)
                {
                    ...
                    for (int i = 0; i < parts.Length; i++)
                    {
                        ...
                        if (p.Length == 0) continue;
                                                ...
                                                bool ok = p.StartsWith("ALDOX-", StringComparison.OrdinalIgnoreCase)
                               || p.StartsWith("CULKCVD-", StringComparison.OrdinalIgnoreCase)
                               || p.StartsWith("CUSILPE-", StringComparison.OrdinalIgnoreCase)
                               || p.StartsWith("DARC-", StringComparison.OrdinalIgnoreCase)
                               || p.StartsWith("HKG-", StringComparison.OrdinalIgnoreCase)
                               || p.StartsWith("SILPE-", StringComparison.OrdinalIgnoreCase);
                ...
                noticeSb.Append("</div>");
                phNewFab24Notice.Controls.Add(new Literal { Text = noticeSb.ToString() });
                ...
                foreach (var section in sections)
                {
                    ...
                    // 只有第一個區塊放 input（避免 id 重複）；但它會共用過濾所有區塊
                    if (string.Equals(section, "ALDOX", StringComparison.Ordinal))
                    {
                        sb.Append("<div class='title-right'>")
                          .Append("<span class='search-label'>Search</span>")
                          .Append("<span class='search-label'>LOTID</span><input id='qLot' type='text' placeholder='contains...' />")
                ...
                phNewFab24Table.Controls.Add(new Literal { Text = sb.ToString() });
...
";
        ...
        using (SqlCommand cmd = new SqlCommand(sql, conn))
        {
            ...
            using (SqlDataReader reader = cmd.ExecuteReader())
            {
                ...
                foreach (var r in rows)
                {
                    ...
                    string eqpidRaw = r["EQPID"] ?? "";
                    string eqpid = NormalizeEqpid("ULKCVD", eqpidRaw) ?? eqpidRaw;
                ...
                foreach (var sec0 in new[] { "ULKCVD" })
                {
                    trendAll.Add(sec0, new System.Collections.Generic.Dictionary<string, int>(StringComparer.Ordinal));
                    trendP14.Add(sec0, new System.Collections.Generic.Dictionary<string, int>(StringComparer.Ordinal));
                    trendP56.Add(sec0, new System.Collections.Generic.Dictionary<string, int>(StringComparer.Ordinal));
                                ...
                                foreach (var sec in new[] { "ULKCVD" })
  {
                ...
                js.Append("],seriesAll:{");
                AppendTrendSeries(js, labels, trendAll, new[] { "ULKCVD" });
                js.Append("},seriesP14:{");
                AppendTrendSeries(js, labels, trendP14, new[] { "ULKCVD" });
                js.Append("},seriesP56:{");
                AppendTrendSeries(js, labels, trendP56, new[] { "ULKCVD" });
                js.Append("}};</script>");
    ...
    private static string ToShortToolLabel(string section, string tool)
    {
        ...
        // section -> initial (merged from all 4 source files)
        string initial = null;
        ...
        else if (string.Equals(section, "ALDOX", StringComparison.OrdinalIgnoreCase)) initial = "A";
        else if (string.Equals(section, "CULKCVD", StringComparison.OrdinalIgnoreCase)) initial = "C";
        else if (string.Equals(section, "CUSILPE", StringComparison.OrdinalIgnoreCase)) initial = "U";
        else if (string.Equals(section, "DARC", StringComparison.OrdinalIgnoreCase)) initial = "D";
        else if (string.Equals(section, "HKG", StringComparison.OrdinalIgnoreCase)) initial = "H";
        else if (string.Equals(section, "SILPE", StringComparison.OrdinalIgnoreCase)) initial = "S";
    ...
    private static string GetSectionFromEqpid(string eqpid)
    {
        ...
        // 注意：longer prefixes first to avoid CULKCVD/CUSILPE shadowing
        ...
        for (int i = 0; i < parts.Length; i++)
        {
            ...
            if (p.StartsWith("CULKCVD-", StringComparison.OrdinalIgnoreCase)) return "CULKCVD";
            if (p.StartsWith("CUSILPE-", StringComparison.OrdinalIgnoreCase)) return "CUSILPE";
            ...
            if (p.StartsWith("ULKCVD-", StringComparison.OrdinalIgnoreCase)) return "ULKCVD";
            if (p.StartsWith("TEOSPE-", StringComparison.OrdinalIgnoreCase)) return "TEOSPE";
            if (p.StartsWith("ALDOX-", StringComparison.OrdinalIgnoreCase)) return "ALDOX";
            if (p.StartsWith("SACVD-", StringComparison.OrdinalIgnoreCase)) return "SACVD";
            if (p.StartsWith("SILPE-", StringComparison.OrdinalIgnoreCase)) return "SILPE";
            if (p.StartsWith("DARC-", StringComparison.OrdinalIgnoreCase)) return "DARC";
            if (p.StartsWith("HKG-", StringComparison.OrdinalIgnoreCase)) return "HKG";
            if (p.StartsWith("APF-", StringComparison.OrdinalIgnoreCase)) return "APF";
        }

        // 如果沒有 ^，或 Split 後沒有命中，退回用 contains 判斷（避免漏資料）
        if (eqpid.IndexOf("CULKCVD-", StringComparison.OrdinalIgnoreCase) >= 0) return "CULKCVD";
        if (eqpid.IndexOf("CUSILPE-", StringComparison.OrdinalIgnoreCase) >= 0) return "CUSILPE";
        ...
        if (eqpid.IndexOf("ALDOX-", StringComparison.OrdinalIgnoreCase) >= 0) return "ALDOX";
        if (eqpid.IndexOf("SACVD-", StringComparison.OrdinalIgnoreCase) >= 0) return "SACVD";
        if (eqpid.IndexOf("SILPE-", StringComparison.OrdinalIgnoreCase) >= 0) return "SILPE";
        if (eqpid.IndexOf("DARC-", StringComparison.OrdinalIgnoreCase) >= 0) return "DARC";
        if (eqpid.IndexOf("HKG-", StringComparison.OrdinalIgnoreCase) >= 0) return "HKG";
