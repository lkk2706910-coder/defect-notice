...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .topbar .me { margin-left: auto; color: var(--muted); font-size: 13px; }
        .topbar .me .role { background: var(--chip); color: var(--text); padding: 2px 8px; border-radius: 999px; font-size: 11px; margin-left: 6px; }
        .topbar a.logout {
            margin-left: 12px;
            padding: 6px 14px;
        ...
        .topbar a.logout:hover { background: var(--warn-bg); color: var(--warn); border-color: var(--warn-border); }
...
<body>
    <div class="topbar">
        <h1>網頁公版 — 首頁</h1>
        <span class="me">你好,<%= Server.HtmlEncode(CurrentUsername) %></span>
        <a class="logout" href="Home.aspx?op=logout">登出</a>
    </div>

    <div class="wrap">
        <div class="card">
            <h2>歡迎</h2>
            <p>這是 <code>web-template</code> 公版的首頁。系統會先要求登入,
                帳號驗證走 SQL Server 的 <code>Users</code> 表(<code>db-schema.sql</code> 建好的)。</p>
            <p>右下角 <strong>AI</strong> 圓鈕點開就有 LLM 對話視窗(來自 <code>ai-chat-widget</code> 整合),
                所有 chat 請求自動帶 cookie token,後端會驗證身份。</p>
        </div>
        <div class="card">
            <h2>下一步</h2>
            <p>把 <code>Home.aspx</code> 改成你自己的功能畫面。要連 DB 的話,在 code-behind 用:</p>
            <p><code>var rows = DbHelper.QueryRows("SELECT * FROM your_table WHERE x = @p0", value);</code></p>
            <p>要加新頁面,只要新檔最上面寫 <code>if (AuthHelper.ReadCurrent() == null) Response.Redirect("Login.aspx");</code>
                就受保護了。</p>
        </div>
    </div>
    ...
    <script>
        // ============================================================
        //  AI Chat Widget -- talks to Home.aspx?op=chat with cookie auth.
        //  Same UX as the standalone ai-chat-widget but proxy is local.
        // ============================================================
        (function () {
            const PROXY_URL = 'Home.aspx?op=chat';
            const STORAGE_KEY = 'webTemplate.aiSessions';
            const ACTIVE_KEY  = 'webTemplate.aiActiveId';
            ...
            async function send() {
                ...
                try {
                    ...
                    if (data && data.error === 'needLogin') {
                        appendMessage('assistant', '登入逾期,請重新整理頁面登入', 'error');
