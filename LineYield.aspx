...
<body>
    ...
    <script>
        ...
        const COLUMNS_LIGHT = [
            { key: 'bulk',          label: '大宗',                  kind: 'checkbox' },
                    ...
                    if (ctx) {
                        ...
                        messages.push({
                            ...
                            content:
                                ...
                                '\n\n→ **【掛帳偵測(規則 A2,最高優先,違反 = 失敗)】**' +
                                '\n   在進行 light dataset 任何 case 萃取時,你**必須**先掃過整列來源(每一個欄位、每一個儲存格、欄位中夾雜的中文字、註解、備註欄、機台欄、原因欄、任何地方)。' +
                                '\n   只要任何位置**包含「掛帳」二字**(完全比對,不要管它的前後文是什麼、不要管它出現在哪個欄位、不要管它是不是夾在機台名字中間),就**必須**在該 case 的 JSON 加上 "onAccount": "1"。' +
                                '\n   範例:來源欄位是 "CULKCVD-A04\\n掛帳\\nULKCVD-B04" → 你應該輸出 eqpId="CULKCVD-A04\\nULKCVD-B04" (拿掉「掛帳」二字) **且** onAccount="1"。' +
                                '\n   沒有「掛帳」二字 → 不要寫 onAccount(省略,不要填 "0" 或 "")。' +
                                '\n   「掛帳」二字本身**不要**寫進其他欄位(像是 eqpId / reason),只反映在 onAccount。' +
